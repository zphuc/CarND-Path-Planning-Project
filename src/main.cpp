...
#define ID_MID_LANE 1
#define ID_MIN_LANE 0
#define ID_MAX_LANE 2
#define D_DD 4
#define D_DS 30
#define D_DT 0.02
#define MAX_SPEED  49.5
#define D_SPEED  0.224


  // Car's lane. Stating at middle lane.                                        
  int car_lane = ID_MID_LANE;

  // Reference velocity.
  double ref_vel = 0.0; // mph

  h.onMessage([&ref_vel, &car_lane, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy]
...
//////////////////////////////////////////////////////////////////////////////
// Peripheral Environment Prediction
            ...
            for ( int i = 0; i < sensor_fusion.size(); i++ ) {
              ...
              if ( ilane >= (ID_MIN_LANE) && ilane <= (ID_MAX_LANE) ) {
                // Find car speed.
...
// Behavior Planning
            if ( car_frnt ) { // there is a car in front
              if ( !car_left && car_lane > ID_MIN_LANE ) {
                ...
                ref_vel = max(ref_vel-D_SPEED,0.0);
              }
            } else {
...
// Trajectory Generation
            ...
            vector<double> next_wp0 = getXY(car_s + 1.0*D_DS, D_DD*(car_lane+0.5), map_waypoints_s, map_waypoints_x, map_waypoints_y);
            vector<double> next_wp1 = getXY(car_s + 2.0*D_DS, D_DD*(car_lane+0.5), map_waypoints_s, map_waypoints_x, map_waypoints_y);
            vector<double> next_wp2 = getXY(car_s + 3.0*D_DS, D_DD*(car_lane+0.5), map_waypoints_s, map_waypoints_x, map_waypoints_y);
            ...
            for( int i = 1; i < 50 - prev_size; i++ ) {
              double N = target_dist/(D_DT*ref_vel/2.24);
